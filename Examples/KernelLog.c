...
double KernelLog(double *Data, double *Params, double *Jacobian){
    ...
    f[4] = log(1+Params[16]*pow(k3, 2))*f[3];
    ...
    double Residue = F - ((Params[1]/pow(k3, 0)+Params[2]/pow(k3, 2)+Params[3]/pow(k3, 4))*f[0]
                        ...
                        + (Params[13]*pow(k3, 1)+Params[14]/pow(k3, 1)+Params[15]/pow(k3, 3))*f[4])
    ...
    if (Jacobian != NULL){
        ...
        Jacobian[13] = -f[4] * pow(k3, 1) / ((1+Params[0]/pow(k1, 2)) * (1+Params[0]/pow(k2, 2)));
        Jacobian[14] = -f[4] / pow(k3, 1) / ((1+Params[0]/pow(k1, 2)) * (1+Params[0]/pow(k2, 2)));
        Jacobian[15] = -f[4] / pow(k3, 3) / ((1+Params[0]/pow(k1, 2)) * (1+Params[0]/pow(k2, 2)));

        Jacobian[16] = -f[3] * (Params[13]*pow(k3, 1)+Params[14]/pow(k3, 1)+Params[15]/pow(k3, 3)) / (pow(k3, -2)+Params[16]) / ((1+Params[0]/pow(k1, 2)) * (1+Params[0]/pow(k2, 2)));
    }
...
int main()
{
    ...
    LevenbergMarquardt(KernelLog, Data.Data, Data.DataSize, Data.VariableSize, Params, ParamsSize);
